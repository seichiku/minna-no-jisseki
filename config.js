...
const CONFIG = {
  ...
  KPI: {
    ...
    // 2026-08-21: 篠田・中谷が抜けていたのを修正（有山=管理部は成約対象外のため除外）
    // 2026-08-22: 竹中さんをストックからも非表示（本人指示）
    STAFF: ['石本', '加藤', '白田', '植田', '篠田', '中谷'],
    HIDE_STAFF: ['竹中'],   // LPに表示しない施術者（個人ランキング・各院の個人カード。2026-08-21 竹中さん指示）
    // 損益分岐ライン（2026-09-17 竹中指示: 3店舗合計 688万円/月）。ペースチャートに赤の点線で表示
    // 院別は CLINICS に金額(円)を入れれば固定値。null なら TOTAL を各院の月予算比で按分して表示（「按分」と注記）
    BREAK_EVEN: {
      TOTAL: 6880000,
